...
import { ProviderConfig } from '../../scripts/controllers/network/chain-provider';
import { getEthQueryFromProviderConfig } from '../../scripts/controllers/network/provider-api-tests/helpers';
import {
  fetchGasEstimates,
...
> {
  ...
  async getGasFeeEstimatesAndStartPolling(
    ...
    if (this.pollTokens.size === 1) {
      await this._fetchGasFeeEstimateData({ provider });
      this._poll({ provider });
    }
  ...
  async _fetchGasFeeEstimateData(
    ...
    let { currentChainId, ethQuery } = this;
    if (options.provider) {
      currentChainId = options.provider.chainId;
      ethQuery = getEthQueryFromProviderConfig(options.provider);
    }
